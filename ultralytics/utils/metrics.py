...
def bbox_iou(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=False, EIoU=False, SIoU=False, ShapeIoU=False, PIoU=False, PIoU2=False, RIOU=False, eps=1e-7, scale=0.0, Lambda=1.3):
    """
    ...
    # IoU
    iou = inter / union
    if CIoU or DIoU or GIoU or EIoU or SIoU or ShapeIoU or PIoU or PIoU2 or RIOU:
        cw = b1_x2.maximum(b2_x2) - b1_x1.minimum(b2_x1)  # convex (smallest enclosing box) width
        ch = b1_y2.maximum(b2_y2) - b1_y1.minimum(b2_y1)  # convex height
        if CIoU or DIoU or EIoU or SIoU or PIoU or PIoU2 or RIOU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            ...
            if CIoU:  # https://github.com/Zzh-tju/DIoU-SSD-pytorch/blob/master/utils/box/box_utils.py#L47
                ...
                maxwh1 = torch.max(w1, h1)
                maxwh2 = torch.max(w2, h2)
                c2 = (maxwh1 + maxwh2 + torch.sqrt(rho2) + eps).pow(2)
                v = (4 / math.pi ** 2) * (torch.atan(w2 / h2) - torch.atan(w1 / h1)).pow(2)
                with torch.no_grad():
                    alpha = v / (v - iou + (1 + eps))
                return iou - (rho2 / c2 + v * alpha)  # RIoU
            elif EIoU:
...
class WiseIouLoss(torch.nn.Module):
    ...
    def forward(self, pred, target, ret_iou=False, ratio=1.0, d=0.0, u=0.95, **kwargs):
        self._fget = {
            ...
            'iou': lambda: (1 - get_inner_iou(pred, target, xywh=False, ratio=ratio).squeeze()) if self.inner_iou else (1 - self['s_inter'] / self['s_union'] if self.focaler_iou else 1 - ((self['s_inter'] / self['s_union'] - d) / (u - d)).clamp(0, 1)),
        }
