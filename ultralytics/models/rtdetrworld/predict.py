...
class RTDETRPredictor(BasePredictor):
    ...
    def postprocess(self, preds, img, orig_imgs):
        ...
        results = []
        outputs = [torch.zeros((0, 6), device=bboxes.device)]
        for i, bbox in enumerate(bboxes):  # (300, 4)
            ...
            if self.args.classes is not None: #指定检测特定目标
                idx = (cls == torch.tensor(self.args.classes, device=cls.device)).any(1) & idx
            pred = torch.cat([bbox, score, cls], dim=-1)[idx]  # filter
            # IOUNMS
            s = pred[:, 4]
            c = pred[:, 5:6] * (0 if self.args.single_cls else max_wh)  # classes
            b = pred[:, :4] + c
            ...
            results.append(Results(orig_img, path=img_path, names=self.model.names, boxes=outputs))
        return results
