...
from ultralytics.data.augment import Compose, Format, v8_transforms, RandomLoadText
...
class RTDETRValidator(DetectionValidator):
    ...
    def postprocess(self, preds):
        ...
        max_wh=7680
        bs, _, nd = preds[0].shape # [b, 300, 4+nc]
        bboxes, scores = preds[0].split((4, nd - 4), dim=-1)
        bboxes *= self.args.imgsz
        outputs = [torch.zeros((0, 6), device=bboxes.device)] * bs #[b,0,6]
        for i, bbox in enumerate(bboxes):  # (300, 4) 对b进行索引吗
            ...
            # IOUNMS
            s = pred_filtered[:, 4]
            c = pred_filtered[:, 5:6] * (0 if self.args.single_cls else max_wh)  # classes
            b = pred_filtered[:, :4] + c
            oi = torchvision.ops.nms(b, s, self.args.iou)
            outputs[i] = pred_filtered[oi]
