...
class DETRLoss(nn.Module):
    ...
    def _get_loss_class(self, pred_scores, targets, gt_scores, num_gts, postfix=''):
        ...
        one_hot = one_hot[..., :-1]
        #将每个预测框与正式框的IOU值 乘以正式对应正式框类别的onehot编码 query还是300  
        # gt_scores也只有idx索引有值，最后得到的[b, query, num_classes]也是部分有值
        # 但pred_scores是300个里面都有值的，这种设计很奇怪吧 承袭之前的设计，但是卷积这样是合理的，对于decoder合适吗？
        ...
        if self.sl or self.emasl:
            ...
            if num_gts:
                if self.svfl:
                    loss_cls = self.vfl(pred_scores, gt_scores, one_hot, auto_iou)
                else:
    ...
    def _get_loss_bbox(self, pred_bboxes, gt_bboxes, postfix=''):
        ...
        if self.use_wiseiou:
            ...
            loss[name_giou] = 1.0 - bbox_iou(pred_bboxes, gt_bboxes, xywh=True, RIOU=True)
    ...
    def _get_loss_aux(self,
        ...
        if pred_scores is None:
            for i, aux_bboxes in enumerate(pred_bboxes):
    ...
    def _get_loss(self,
        ...
        bs, nq = pred_bboxes.shape[:2] #[b, query]
        ...
        # 用IOU值乘以正式框编码作为真实值，回归任务结果依旧影响分类任务 （yolo中没有这种设计）
        if len(gt_bboxes):
    ...
    def forward(self, pred_bboxes, pred_scores, batch, postfix='', **kwargs):
        ...
        Args:
            ...
            batch (dict): A dict includes:
                ...
                gt_groups (List(int)): a list of batch size length includes the number of gts of each image. 这个批次中有多少组GT v3设置为4
        ...
        # 这里计算loss，并采用匈牙利分配
        if pred_scores is None:
            total_loss = self._get_loss(pred_bboxes[-1], #[b, query, 4]
                                        pred_scores, #[b, query, nc]
                                        gt_bboxes,
                                        gt_cls,
                                        gt_groups,
                                        postfix=postfix,
            ...
            if self.aux_loss:
                total_loss.update(
                    self._get_loss_aux(pred_bboxes[:-1], pred_scores, gt_bboxes, gt_cls, gt_groups, match_indices,
                                    postfix))

            return total_loss
        
        total_loss = self._get_loss(pred_bboxes[-1], #[b, query, 4]
                                    pred_scores[-1], #[b, query, nc]
                                    gt_bboxes,
                                    gt_cls,
                                    gt_groups,
                                    postfix=postfix,
...
class RTDETRDetectionLoss(DETRLoss):
    ...
    def forward(self, preds, batch, dn_bboxes=None, dn_scores=None, dn_meta=None):
        ...
        """
        pred_bboxes, pred_scores = preds # (7, bs, 300, 4) 和 (7, bs, 300, nc)
        ...
        if dn_meta is not None:
            ...
            # Compute the denoising training loss 甚至还有计算去噪损失的
...
# RTDETR的loss函数
class RTDETRDetectionlocationLoss(DETRLoss):
    """
    Real-Time DeepTracker (RT-DETR) Detection Loss class that extends the DETRLoss.
    ...
    def forward(self, preds, batch, dn_bboxes=None, dn_scores=None, dn_meta=None):
        ...
        Returns:
            (dict): Dictionary containing the total loss and, if applicable, the denoising loss.
        """
        pred_bboxes, pred_scores = preds # (7, bs, 300, 4) 和 (1, bs, 300, nc)
        total_loss = super().forward(pred_bboxes, pred_scores, batch)
        #print(total_loss)
        # Check for denoising metadata to compute denoising training loss
        if dn_meta is not None:
