...
# op为SS2D操作， self.norm = norm_layer(hidden_dim), drop_path概率init默认为0，意味着不会有self.norm(self.op(input)被丢弃
    def _forward(self, input: torch.Tensor):
        # 如果self.ssm_branch = 2大于0，则经过一个线性，再一个SS2D加起来，post_norm默认为flase，norm为论文中LN
        if self.ssm_branch:
            if self.post_norm:
                x = input + self.drop_path(self.norm(self.op(input)))
            else:#选这个
                x = input + self.drop_path(self.op(self.norm(input)))
        # 如果self.mlp_branch = 4大于0，则经过一个线性，再一个MLp加起来，post_norm默认为flase
        if self.mlp_branch:
            if self.post_norm:
